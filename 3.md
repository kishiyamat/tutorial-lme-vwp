...
LMを組むには `data.frame`と`formula`、`model` が必要で、
GLMを組むには観測できる値が従う従う確率分布を`family`で指定すること、
そしてGLMの結果を解釈をしやすくするためにはリンク関数があることを学びました。
以下のコードを見て思い出しましょう。
...
なぜ`binomial`かといえば、`Reaction`とことなり、
`Correct`は0/1で二項分布に従うからでしたね。
また結果の解釈には「リンク関数」が必要でした。
`binomial`の場合、`logit`関数が相当したことを思い出しましょう。
解釈に不安が残る場合は前回の内容を再度実行して
コメントの `exp(ax)+exp(b)` や `p/(1-p)` が何を示すのかを
思い出しましょう。
...
最後に <u>(iii) glmer を用いた一般化線形混合モデル</u> を実際に作成してみます。
...
被験者ごとに傾きや切片の傾向は「全体的ななんとなくの方向」はあるものの、
...
そうではなく、<u>全体的ななんとなくの方向</u> を「固定効果」、
...
これに対して、もし <u>全体的ななんとなくの方向 (固定効果)</u> と
...
一旦 `coef(m)$Subject`と `ranef(m)$Subject` は無視して、
以下のコードを実行してみましょう。
