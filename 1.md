...
以下、先ほどの例でで出てきた
$y = a_1x_1 + a_2x_2 + b$ のような「線形モデル」の作成を
通じて統計モデルとその基本要素である
(i) `data.frame`
(ii) `formula`
(iii) `model`
...
Rには`()`を右に持つ「関数」というのがあり、
例として `head()` や `summary()` といった関数は
`data.frame` を `()` の中にもらうと中身を教えてくれます。
以下、`sleepstudy` という `data.frame` を既に読み込んであるので、
`head()` や `summary()` の `()` に入れて動作を確認してみましょう。
なお、 `head()` は受け取ったデータの最初の6件をデフォルトで返し、
...
ちゃんと `head()` と `summary()` の機能とデータフレームの概念を掴めたでしょうか。
...
なおこのデータは以下のような設定で，
`Reaction`, `Days`, `Subject` という3つの列があります。
